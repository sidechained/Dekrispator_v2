This is a fork of the original Dekrispator project, found [here](https://github.com/MrBlueXav/Dekrispator_v2), which allows a STM32F407VG Discovery board to be used as a standlone synthesiser, as controlled by a Korg NanoKontrol over USB MIDI.

In this version, I swapped out the original synthesiser code for C code generated by [Faust](https://faust.grame.fr]) and updated the MIDI controller to the NanoKontrol2, making this a useful environment for developing embedded synthesisers. Any MIDI controller can be used with minimal changes needed in the code and when the project gets beyond the prototyping stage, the MIDI control can relatively simply be swapped out for custom electronics.
...
- rebuild the project, flash to the board and have fun!

## 1. Writing/Exporting Faust Code

Write or copy some simple Faust code using the [Faust Online IDE](https://faustide.grame.fr). For our tests we used the following code snippet, which can be found in [Synth/mydsp-faustcode.dsp](Synth/mydsp-faustcode.dsp).
...
- Q: do these have to use HAL library to work well with Dekrispator? A: Ideally, yes
...
- Add include for CInterface to makefile instead of in local repo
- output seems a bit 'crispy'
- strange clicks and pops within first few seconds of running
- Note that I did not have to include "soundGen.h" from "MIDI_application.c"
